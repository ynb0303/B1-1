...
현재 상태: 구현과 로컬 Chrome 검증 완료. 체크된 구현 항목 중 모든 분기까지 자동 검증한 것은 아니며, 범위는 README에 기록했습니다. 본인의 설명 연습과 GitHub 업로드·Pages 배포 및 배포 후 확인은 남아 있습니다. 스크린샷은 로컬 실제 API 화면입니다.
...
## 9. 배포와 제출
- [ ] Git 저장소 초기화 및 GitHub 저장소 준비
- [ ] GitHub Pages 배포
- [ ] 외부에서 접속 가능한 배포 URL 확인
- [ ] 배포 사이트에서 반응형, 햄버거, 다크 모드, 스크롤 확인
- [ ] 배포 사이트에서 GitHub API와 폼 유효성 검사 확인
